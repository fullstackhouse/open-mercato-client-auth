...
| `OAUTH_WEB_REDIRECT_ORIGINS` | Comma-separated origins that web flows may redirect to after sign-in (for SPAs served from another origin). Absolute `redirect` URLs are honored only when their origin is listed; a `redirect` naming any other origin is refused with `400` at `/oauth/<provider>/init`, and a relative one falls back to same-app path-only redirects. An entry may carry a single `*` standing for exactly one host label, so `https://*.example.com` admits every tenant subdomain and nothing deeper. Empty by default. |
